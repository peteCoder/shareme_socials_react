...
const PinDetails = ({ user }) => {
  ...
  if (!pinDetail) return <Spinner message="Loading Pin Content! " />
    ...
    <>
      <div className='flex xl:flex-row flex-col m-auto bg-white' style={{ maxWidth: '1500px', borderRadius: '32px' }}>
        ...
        <div className="w-full p-5 flex-1 xl:min-w-620">
          ...
          <Link to={`user-profile/${pinDetail.postedBy?.id}`} className="flex gap-2 items-center mt-5 bg-white rounded-lg z-100" >
            <img
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <Link to={`user-profile/${pinDetail.postedBy?.id}`}  >
              <img
                className='w-10 h-10 rounded-full object-cover cursor-pointer bg-white'
                src={pinDetail.postedBy?.image}
                alt="user-profile"
              />
            </Link>
