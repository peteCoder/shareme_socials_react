...
const CreatePin = ({ user }) => {
  ...
  const uploadImage = (e) => {
    ...
    const { type, name } = e.target.files[0]; // Destructured `selectedFile` 
          ...
          <div className="flex justify-center items-center border-2 border-dotted border-gray-300 p-3 w-full h-420">
            ...
            {!imageAsset ? (
              ...
              <div className='relative h-full '>
                <img src={imageAsset?.url} alt="uploaded-pick" className='h-full w-full' />
                <button
                  type='button'
                  className='absolute bottom-3 right-3 p-3 rounded-full bg-white text-xl cursor-pointer hover:shadow-md outline-none transition-all duration-500 ease-out'
                  onClick={() => {
                    setImageAsset(null)
                  }}
                >
